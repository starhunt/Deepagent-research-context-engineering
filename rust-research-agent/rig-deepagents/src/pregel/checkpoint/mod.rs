...
/// - Retry counts per vertex (for fault tolerance)
    ...
    S: WorkflowState,
{
    ...
    /// Retry attempt counts per vertex (for fault tolerance across resumes)
    ///
    /// This field is stored as a first-class field rather than in metadata
    /// to ensure type safety and avoid double-serialization overhead.
    /// (Gemini/Qwen code review feedback)
    #[serde(default)]
    pub retry_counts: HashMap<VertexId, usize>,
    ...
    S: WorkflowState,
{
    ...
    /// Create a checkpoint with retry counts
    ///
    /// This is the preferred constructor when checkpointing from a running runtime,
    /// as it preserves retry state across checkpoint boundaries.
    pub fn with_retry_counts(
        workflow_id: impl Into<String>,
        superstep: usize,
        state: S,
        vertex_states: HashMap<VertexId, VertexState>,
